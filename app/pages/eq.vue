...
import { CHANNEL_LAYOUTS, type LayoutKey } from "~/utils/channelLayouts"
import {
  apiCellsToBands,
  bandsToApiCells,
...
type ApiPreset = { name: string; eqListe: Record<string, ApiCell[]>, rate: number, configLayouts: string }

const fs = ref(48000)
const curvePoints = ref(2000)
const dbRange = ref(36)

const selectedLayout = ref<LayoutKey>("")
...
const presetsOriginal = ref<ApiPreset[]>([]) // snapshot pour reset
...
async function fetchEq() {
  ...
  loadError.value = ""
  try {
    // adapte selon ton wrapper http :
    // - soit res.data contient le tableau
    // - soit res est déjà le tableau
...
async function postEqPreset() {
  ...
  // ✅ payload conforme à ton API
  const body: ApiPreset = {
    name: p.name,
    rate: fs.value, // ou p.rate si tu veux verrouiller
    configLayouts: selectedLayout.value, // ou p.configLayouts
    eqListe: p.eqListe
  }

  try {
    // 1) si ton wrapper http supporte post
...
watch(selectedPreset, (p) => {
  ...
  // layout depuis l'API
  const layout = p.configLayouts as LayoutKey
  if (layout && (layout in CHANNEL_LAYOUTS)) {
    selectedLayout.value = layout
  } else {
    // fallback si l'API renvoie un layout inconnu
    selectedLayout.value = "stereo"
  }
}, { immediate: true })

const layoutKeys = Object.keys(CHANNEL_LAYOUTS) as LayoutKey[]
const layoutItems = computed(() =>
  layoutKeys.map(k => ({ label: k, value: k }))
)

// ----- Channels available (layout order ∩ eqListe keys) -----
const channelItems = computed(() => {
  const p = selectedPreset.value
  if (!p) return []

  const keys = Object.keys(p.eqListe || {})
  const order = CHANNEL_LAYOUTS[selectedLayout.value]?.order || []

  const final = order.length
    ? order.filter(ch => keys.includes(ch))
    : keys
...
watch([channelItems, selectedPresetName], () => {
...
// ----- Bands state (réel, modifiable) -----
const bands = ref<UiBand[]>([])

function ensureSixCells() {
  // on veut toujours les cells 1..6 présentes
  ...
  // garder seulement 1..6
  bands.value = bands.value
    .filter(b => b.cell >= 1 && b.cell <= 6)
...
function loadBandsFromApi() {
  const p = selectedPreset.value
  if (!p) return
  const cells = p.eqListe?.[selectedChannel.value] || []
...
// Quand tu modifies bands -> on ré-écrit dans le preset courant (local)
watch(bands, () => {
  const p = selectedPreset.value
  if (!p) return
  const channelId = p.eqListe?.[selectedChannel.value]?.[0]?.["@channel"] // ex "1"
  p.eqListe[selectedChannel.value] = bandsToApiCells(bands.value, channelId)
}, { deep: true })
// watch(saving, () => {
//   scheduleDraw()
// })
...
function toggleBand(cell: number, v: boolean) {
  ...
  if (!b) return
  b.enabled = v
  // si on active, assure une freq valide
...
function resetChannel() {
  ...
  if (!orig) return
  const cells = orig.eqListe?.[selectedChannel.value] || []
  bands.value = apiCellsToBands(cells)
  ensureSixCells()
}
...
function addBand(type: UiBand["type"]) {
  // on remplit la 1ère cell OFF, sinon on écrase la 6
  ensureSixCells()
  const target = bands.value.find(b => !b.enabled) || bands.value.find(b => b.cell === 6)
  if (!target) return
...
const apiJson = computed(() => {
  ...
  const out = {
    ...
    eq: p.eqListe?.[selectedChannel.value] || []
  }
...
function draw() {
  ...
  // ✅ on dessine en coordonnées CSS (plus simple pour texte)
  ...
  // --- Grille fréquences + labels en bas ---
      ...
      <div class="w-full lg:max-w-12xl py-6 sm:py-8 lg:py-12 space-y-6">
          ...
          >
            ...
            <div class="mt-3 space-y-4">
              ...
              <div class="m-0">
                ...
                <input
                  type="range" min="0.10" max="63" step="0.01"
                  class="w-full accent-current h-1.5 range-primary-0"
                  :value="b.q"
        ...
        <UCard>
          <div class="flex items-center justify-between gap-2">
            <div>
              <div class="font-medium">Export API (par canal)</div>
              <div class="text-xs text-dimmed">Format: @cell @EQon @EQtype @freq @Q @dblevel (+ @channel)</div>
            </div>
            <div class="flex items-center gap-2">
              <UButton size="sm" color="neutral" variant="soft" @click="copyApi">Copier</UButton>
            </div>
          </div>

          <pre class="mt-4 text-xs max-h-[420px] rounded-md border border-default/60 p-3 bg-elevated/30 overflow-auto whitespace-pre-wrap break-words">{{ apiJson }}</pre>
        </UCard>

        <!-- Copier depuis un autre canal -->
        <UCard>
          <div class="flex items-center gap-2">
            <div class="text-sm text-dimmed">Copier les réglages depuis :</div>
            <USelect
              :items="channelItems.filter(i => i.value !== selectedChannel)"
              placeholder="Choisir un canal"
              class="min-w-[160px]"
              @update:model-value="(v:string)=>copyFromChannel(v)"
            />
          </div>
        </UCard>
      </div>
