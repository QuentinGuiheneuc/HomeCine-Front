...
function ensureSubConfigs() {
  ...
  if (!next.backend) next.backend = 'pipe'
  if (!next.bitrate) next.bitrate = '320'
  if (next['enable-volume-normalisation'] === undefined) next['enable-volume-normalisation'] = false
  if (!next['initial-volume']) next['initial-volume'] = '100'
  if (!next['device-type']) next['device-type'] = 'avr'
  if (!next['path-audio']) next['path-audio'] = '/tmp/'
  if (!next.frames_per_buffer) next.frames_per_buffer = 1024
...
const bitrateItems = computed(() => typeStreamOptions.spotify?.bitrateItems || [])
const deviceTypeItems = computed(() => typeStreamOptions.spotify?.Device_typeItems || [])
const framesPerBufferItems = computed(() => typeStreamOptions.spotify?.frames_per_bufferItems || [])
</script>

<template>
  <UPageCard variant="subtle" :ui="{ container: 'p-4 space-y-4' }">
    <h3 class="font-semibold">Configuration Spotify</h3>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="text-sm text-dimmed">Backend</label>
        <UInput v-model="localCfg.backend" placeholder="pipe" />
      </div>
      ...
      <div>
        ...
        <UInput v-model="localCfg['initial-volume']" placeholder="100" />
      </div>
      ...
      <div class="md:col-span-2">
        ...
        <UInput v-model="localCfg['path-audio']" placeholder="/tmp/" />
      </div>
